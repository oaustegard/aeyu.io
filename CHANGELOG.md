...
- **Athletes with no Strava profile picture saw a broken-image glyph instead of an avatar** — Strava returns `athlete.profile` as the bare relative path `avatar/athlete/large.png` for anyone who has not uploaded a photo. `StickyHeader` branched on truthiness, so that sentinel took the `<img>` path, the browser resolved it against aeyu.io, and the 404 drew the glyph; the initial-letter fallback sitting right there never ran. `resolveAvatarUrl()` in a new `src/avatar.js` treats anything that is not an absolute http(s) URL as no avatar, applied at read time so existing sessions are fixed without re-authorising. A single `Avatar` component now backs all three render sites and carries an `onError` fallback for the failures a string check cannot predict — deleted CDN objects, hotlink blocks, offline devices. Also fixes a throw on an empty first name, and the dropdown avatar, which had no fallback at all.
- **Reference Points form no longer re-renders the whole dashboard on every keystroke** — its six fields were bound to module-level signals, and reading a signal's `.value` inside `Dashboard()` subscribes the entire component: 14 characters typed produced 14 full dashboard renders, ride cards and charts included. Reported from mobile as the field losing focus after each character. `ReferencePointForm` is now its own component holding those fields in `useState`, and a keystroke re-renders only the form. Separately, the settings inputs computed to 12px, which makes iOS Safari zoom the page on focus; `app.css` now sets 16px under `@media (pointer: coarse)` so touch devices stop triggering it and desktop keeps the compact type scale.
...
- **CP as a Power Curve chart row + CP Milestone award** — Critical Power now appears as a labelled bar in the Power Curve chart (not just in the header), with a distinctive striped fill that distinguishes the model-fit parameter from the raw duration bests around it. A new **CP Milestone** award fires when a ride pushes the all-time CP fit across a 50W threshold (150 / 200 / 250 / 300 / 350 / 400W), mirroring the existing FTP Milestone but anchored on the physiologically grounded CP rather than the 95%-of-20-min FTP proxy.
