...
const FAQ_ENTRIES = [
  ...
  },
  {
    ...
    answer: "The app automatically detects recurring routes (ride clusters) by comparing which segments appear on each ride. Two activities are considered the same route if they have similar total distance (within 30%) and at least 50% segment overlap. This powers the Route Season First award, and each route shows trend charts for average speed and power over time. By default, routes are named after the most common activity name in the cluster. To set a custom name, save any ride from the cluster as a Route in Strava and give it the name you want — the app syncs your saved Strava routes and uses those names. You can also manually sync routes from Settings. Route chips appear below the search bar; tap one to see its trend chart and filter to those rides. When searching, the app also clusters matching results to surface repeated routes in your search results.",
  },
  {
    id: "touch-tooltips",
...
const COACH_ARTIFACT_URL = "https://claude.ai/public/artifacts/52d3ecd0-ea77-4646-87c3-54780ce258d7";
...
function renderRouteTrendChart(route) {
  return html`<${TrendChart} rides=${route.rides || []} />`;
}

function renderGroupRideTrendChart(group) {
  return html`<${TrendChart} rides=${group.rides} />`;
}
          ...
          <div class="rounded-xl shadow-xl w-full max-w-lg p-6 my-4" style="background: var(--surface); border: 1px solid var(--border);">
            ...
            <div style="border-color: var(--border-light);" class="divide-y">
              ...
              <details class="group py-3">
                ...
                <div class="pt-3 pb-1" style="font-family: var(--font-body); font-size: 0.875rem; color: var(--text-secondary);">
                  <p>The app automatically detects recurring routes (ride clusters) by comparing which segments appear on each ride. Two activities are considered the same route if they have similar total distance (within 30%) and at least 50% segment overlap. This powers the Route Season First award — rather than listing a Season First for every segment on a familiar ride, it collapses them into a single route-level award.</p>
